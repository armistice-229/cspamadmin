...
router.get("/progress", async (req, res) => {
  try {
    ...
    // 3️⃣ Total encaissé (transactions "entree") pour CETTE année scolaire uniquement
    const transactions = await Transaction.aggregate([
      { $match: { type: "entree", anneeScolaire } },
...
router.get("/stats", async (req, res) => {
  try {
    ...
    // 3️⃣ Solde de caisse de l'année scolaire ACTIVE uniquement (visible
    // uniquement si rôle autorisé). Le solde repart donc à zéro chaque rentrée :
    // seules les entrées/sorties de l'année en cours sont comptées.
    let solde = null;
...
router.get("/stats-caisse", async (req, res) => {
  try {
    // Ces graphiques suivent la même logique que le solde : ils repartent à
    // zéro chaque année scolaire (sinon les courbes mélangeraient plusieurs
    // rentrées et n'auraient plus de sens pour l'école).
    const anneeScolaire = req.query.annee || (await getAnneeActive());
    ...
    const depensesCategories = await Transaction.aggregate([
      { $match: { type: "sortie", anneeScolaire } },
      {
        $group: {
          _id: "$categorie",
          montant: { $sum: "$montant" }
...
router.get("/effectifs", async (req, res) => {
  try {
    // Par défaut, effectifs de l'année scolaire active (pas un mélange de toutes les années)
    const annee = req.query.annee || (await getAnneeActive());
    const filtre = { anneeScolaire: annee };
