...
// ✅ Récupérer les élèves d’une classe donnée (pour une année scolaire donnée)
...
router.get("/:eleveId", async (req, res) => {
  try {
    ...
    // ⚡ Récupérer les frais de scolarité liés à la classe ET à l'année de l'élève
    const frais = await FraisScolarite.findOne({ classe: eleve.classe, anneeScolaire: eleve.anneeScolaire });
    if (!frais) {
      return res.status(404).json({ error: "Aucun frais trouvé pour cette classe et cette année" });
    }

    // ⚡ Récupérer toutes les transactions de type "entree" (paiements) de CETTE fiche élève
    // (une fiche = une année, donc pas besoin de filtre supplémentaire par année ici)
